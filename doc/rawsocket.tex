...
senden und alle Nachrichten, die auf einem CAN-Bus übertragen
werden, zu lesen. Geöffnet wird ein RAW-Socket durch

\begin{code}
s = socket(PF_CAN, SOCK_RAW, 0);
\end{code}

Der geöffnete Socket muss zunächst mittels \man{bind}{2} an einen CAN-Bus
gebunden werden.  Dabei spielen die für Transportprotokolle benötigten
Adress-Elemente \verb|tx_id| und \verb|rx_id| in der Struktur
\verb|struct sockaddr_can| keine Rolle. Der 
folgende Code bindet den geöffneten Socket s an das CAN-Interface
   \verb|can1|:
...
Es können nun mit \man{read}{2} alle auf dem Bus empfangenen CAN-Frames
gelesen und mit \man{write}{2} beliebige CAN-Frames gesendet werden.\\

Die mit \man{read}{2} und \man{write}{2} übertragenen Daten haben die
Struktur \verb|struct can_frame|. Jeder zu sendende CAN-Frame muss 
mit \emph{einem} Aufruf von \man{write}{2} übergeben werden und
empfangene CAN-Frame müssen mit \emph{einem} Aufruf von \man{read}{2}
gelesen werden.\\
...
über einen solchen RAW-Socket muss dann über \man{sendmsg}{2} erfolgen.\\

Der RAW-Socket bietet eine einfache Filterfunktion mit der Bereiche
von CAN-IDs aus dem Datenstrom ausgefiltert werden können. Dazu kann
...
Der Filter läßt sich mit \verb+rfilter.can_id |= CAN_INV_FILTER;+ auch
invertieren, wodurch in diesem Fall die CAN-IDs von 0x200 - 0x2FF
nicht durchgelassen werden würden.\\
...
Für die Anwendung der Filterfunktion muss die Include-Datei \verb+raw.h+
eingebunden werden. Eine Veränderung des Filters zur Laufzeit ist über
weitere Aufrufe von \man{setsockopt}{2} möglich.

\newpage
\subsection{Testprogramme}
\begin{description}
\item[candump] ist ein Programm, das über den RAW-Socket CAN-Frames
von einem oder mehreren CAN-Device(s) einliest und in lesbarer Form -
 auf Wunsch mit Zeitstempeln - ausgibt. Die beschriebene
 Filterfunktion der RAW-Sockets (s.o.) ist über Komandozeilenparameter
...
{\large Beispiele für die Benutzung von {\tt candump}:}\\
\\
...
Null. Ausgabe im ASC-Format. Die Kanalnummern werden um 1 erhöht
...
CAN-Frames dieses mit einer um 1 erhöhten CAN-ID wieder aussendet. Es
war für die ersten Versuche mit einem realen CAN-Bus implementiert
worden. Seit dem \LL\ V0.6 ist jedoch die lokale Echofunktionalität
realisiert, so dass \verb+canecho+ nur noch dazu geeignet ist, einen
Volllast-Test auszuführen ...
\end{description}
